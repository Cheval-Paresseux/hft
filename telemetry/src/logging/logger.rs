...
pub struct Logger<const VEC: usize, const STR: usize> {
    pub id: Uuid,
    pub name: &'static str,
...
impl<const VEC: usize, const STR: usize> Logger<VEC, STR> {
    ...
    pub fn scope(name: &'static str, parent_id: Option<Uuid>) -> Self {
        let mut logger = Self::new(name, parent_id);
...
impl<const VEC: usize, const STR: usize> Drop for Logger<VEC, STR> {
    fn drop(&mut self) {
...
pub type HotLogger     = Logger<HOT_VEC_CAP,     HOT_STR_CAP>;
pub type DefaultLogger = Logger<DEFAULT_VEC_CAP, DEFAULT_STR_CAP>;
pub type FullLogger    = Logger<FULL_VEC_CAP,    FULL_STR_CAP>;
